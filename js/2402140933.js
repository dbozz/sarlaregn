...
function ajaxUpdateSR (key1) {
    console.log("Test");
    return new Dexie.Promise(function (resolve, reject) {
        var key1 = Cookies.get('key1');
        $.ajax("update.json.php?key=" + key1 + "&curts=" + version.map(function (v) { return v.ts }), {
            type: 'get',
	    dataType: 'json',
	    error: function (xhr, textStatus) {
		reject(textStatus);
...
function updateLyrics() {
    db.lyrics.orderBy("ts").reverse().limit(1).toArray(function(version) {
        console.log ("Database Version: " + version.map(function (v) { return v.ts }));
        if (navigator.onLine) {
	    ajaxUpdateSR(Cookies.get('key1'));
        }
    });
}
...
function gAnalytics(nr,sb,event) {
    gtag('event', event, {
        ...
        'page_path': "/" + item.nr + "" + sb
    });
}

// Load lyric into div
function getLyric(id) {
//  db.tranaction("rw", db.lyrics, db.bookmarks, () => {
    db.lyrics.where("id").equals(id).each(function(item) {
//		$( '#lyric_head').html('Kopiera <a href=\"javascript:ttc()\">text</a> | <a href=\"javascript:copyLink()\">länk</a>');
...
//		$( '#lyric_head').html('<a href=\"javascript:delete_bookmark(\'' + id + '\',' + item.id + ');\">Ta bort bokmärke</a><br>Kopiera <a href=\"javascript:ttc()\">text</a> | <a href=\"javascript:copyLink()\">länk</a>');
//		var bm = checkBookmark(id);
...
function bookmarks() {
    ...
    bm_list.push("<h2>Bokmärken</h2><p>Tillfälligt inaktiverad");
    db.bookmarks.each(function(bookmark) {
        var link = '<p><a href="javascript:getLyric(\'' + bookmark.lyrics_id + '\');">' + bookmark.excerpt + '</a>';
	bm_list.push(link);
    }).then(function() { 
...
function add_bookmark(id) {
    db.lyrics.where("id").equals(id).each(function(item){
        db.bookmarks.add({lyrics_id: id, excerpt: item.label });
    });
...
// Delete bookmark
function delete_bookmark(lyric_id,id) {
